            ...
            <div class="card">
              ...
              <div class="card-body">
                <ValidationObserver v-slot="{ invalid,handleSubmit }">
                  <form @submit.prevent="handleSubmit(editNutrients)" ref="nutrientsForm" enctype="multipart/form-data">
                    <v-stepper v-model="e1">
                      ...
                      <v-stepper-items>
                        <v-stepper-content step="1">
                          <ValidationProvider name="Besin Adı" rules="required" v-slot="{ errors }">
                          ...
                          <ValidationProvider name="Besin Açıklaması" rules="required" v-slot="{ errors }">
                            <div class="form-group">
                          ...
                          <v-tabs-items v-model="tab">
                            ...
                            <v-tab-item>
                              <v-card flat>
                                <v-card-text>
                                  <div class="row" v-if="data.criteria_values !== null && data.criteria_values !== undefined && data.criteria_values !== '' && data.criteria_values.length > 0" v-for="(input,index) in data.criteria_values">
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 col-xl-4">
                                      <ValidationProvider name="Ölçüt Değeri Adı" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          ...
                                          <select name="criteriaName[]" v-bind:id="'criteria'+index" class="form-control">
                                            ...
                                            <option v-bind:value="criteria.name" :selected="criteria.name === input.title" v-if="data.criterias !== null && data.criterias !== undefined && data.criterias !== '' && data.criterias.length>0" v-for="criteria in data.criterias">{{criteria.name}}</option>
                                          </select>
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                                    ...
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-2 col-xl-2  py-auto my-auto text-center align-bottom">
                                      <button @click.prevent="cloneProperty2" class="btn btn-lg btn-primary text-white rounded-circle align-bottom text-center" role="button">
                                        <i class="fa fa-plus"></i></button>
                                      <button v-if="data.criteria_values.length > 1" @click.prevent="removeProperty2(index)" role="button" class="btn btn-lg btn-danger text-white rounded-circle align-bottom text-center">
                                        <i class="fa fa-times"></i></button>
                                    </div>
                                  </div>
                                </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      options: {
        url: process.env.apiBaseUrl + "panel/nutrients/create-file/"+this.$route.params.id,
        headers: {
  ...
  methods: {
    ...
    retrieveData(url) {
      ...
      this.$axios.get(`${process.env.apiBaseUrl}panel/datatables/${urlParam}?table=nutrients_file&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,email,phone&where_column=nutrients_id&where_value=${this.data._id.$oid}&joins=nutrients_file`, {
        json: true,
    ...
    deleteData(id) {
      this.$axios.delete(process.env.apiBaseUrl + "panel/datatables/delete-file?id=" + id+"&table=nutrients_file", {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
    ...
    isActiveSetter(id) {
      this.$axios.get(process.env.apiBaseUrl + "panel/datatables/is-active-setter?table=nutrients_file&id=" + id, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
    ...
    isCoverSetter(id) {
      this.$axios.get(process.env.apiBaseUrl + "panel/datatables/is-cover-setter?table=nutrients_file&foreign_column=nutrients_id&id=" + id, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
    ...
    editNutrients() {
      ...
      this.$axios.post(process.env.apiBaseUrl + "panel/nutrients/update/" + this.data._id.$oid, formData, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
