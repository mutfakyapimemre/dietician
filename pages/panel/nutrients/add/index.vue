            ...
            <div class="card">
              ...
              <div class="card-body">
                <ValidationObserver v-slot="{ invalid,handleSubmit }">
                  <form @submit.prevent="handleSubmit(saveNutrients)" ref="nutrientsForm" enctype="multipart/form-data">
                        ...
                        <v-stepper-content step="1">
                          ...
                          <v-tabs-items v-model="tab">
                            ...
                            <v-tab-item>
                              <v-card flat>
                                <v-card-text>
                                  <div class="row" v-if="inputs2 !== null && inputs2 !== undefined && inputs2 !== '' && allCriterias !== null && allCriterias !== undefined && allCriterias !== ''" v-for="input2 in inputs2">
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 col-xl-4">
                                      <ValidationProvider v-bind:name="input2[0].label" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          ...
                                          <select name="criteriaName[]" v-bind:id="input2[0].id" v-model="input2[0].value" class="form-control">
                                            <option v-bind:value="criteria.name" v-if="allCriterias !== null && allCriterias !== undefined && allCriterias !== '' && allCriterias.length>0" v-for="criteria in allCriterias">{{criteria.name}}</option>
                                          </select>
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                          ...
                          <button class="btn btn-outline-primary rounded-0 btn-lg" type="submit">Besini Kayıt Et, Görsel
                            Yüklemeye Geç
                          </button>
                        </v-stepper-content>
...
export default {
  ...
  methods: {
    getCriterias(){
      this.$axios.get(`${process.env.apiBaseUrl}panel/nutrients/create`, {
        json: true,
    ...
    retrieveData(url) {
      ...
      this.$axios.get(`${process.env.apiBaseUrl}panel/datatables/${urlParam}?table=nutrients_file&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,email,phone&where_column=nutrients_id&where_value=${this.inputData.id}&joins=nutrients_file`, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
    ...
    saveNutrients() {
      ...
      this.$axios.post(process.env.apiBaseUrl + "panel/nutrients/create", formData, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
        ...
        .then(response => {
          if (response.data.success) {
            ...
            this.$refs.myDropzone.options.url = process.env.apiBaseUrl + "panel/nutrients/create-file/" + response.data.data.$oid
            this.$refs.myDropzone.dropzone.options.url = process.env.apiBaseUrl + "panel/nutrients/create-file/" + response.data.data.$oid
            this.options.url = process.env.apiBaseUrl + "panel/nutrients/create-file/" + response.data.data.$oid
