...
export default {
  ...
  methods: {
    ...
    retrieveData(url) {
      ...
      this.$axios.get(`${process.env.apiBaseUrl}panel/datatables/${urlParam}?table=recipe_categories_file&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,email,phone&where_column=recipe_category_id&where_value=${this.data._id.$oid}&joins=recipe_categories_file`, {
        json: true,
    ...
    editRecipeCategories() {
      ...
      this.$axios.post(process.env.apiBaseUrl + "panel/recipe-categories/update/" + this.data._id.$oid, formData, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
