            ...
            <div class="card">
              ...
              <div class="card-body">
                <ValidationObserver v-slot="{ invalid,handleSubmit }">
                  <form @submit.prevent="handleSubmit(editDiseases)" ref="diseasesForm" enctype="multipart/form-data">
                    <v-stepper v-model="e1">
                      ...
                      <v-stepper-items>
                        <v-stepper-content step="1">
                                ...
                                <v-card-text>
                                  <div class="row" v-if="data.values !== null && data.values !== undefined && data.values !== '' && data.values.length > 0" v-for="(input,index) in data.values">
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-4 col-xl-4">
                                      <ValidationProvider name="Hastalık Değeri Adı" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          <label v-bind:for="'disease'+index">Hastalık Değeri Adı</label>
                                          <input v-bind:id="'disease'+index" type="text" class="form-control" name="diseaseName[]" v-model="input.title">
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-2 col-xl-2">
                                      <ValidationProvider name="Minimum Hastalık Değeri" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          <label v-bind:for="'diseaseMin'+index">Minimum Hastalık Değeri</label>
                                          <input v-bind:id="'diseaseMin'+index" type="text" class="form-control" name="diseaseMin[]" v-model="input.min">
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-2 col-xl-2">
                                      <ValidationProvider name="Maximum Hastalık Değeri" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          <label v-bind:for="'diseaseMax'+index">Maximum Hastalık Değeri</label>
                                          <input v-bind:id="'diseaseMax'+index" type="text" class="form-control" name="diseaseMax[]" v-model="input.max">
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                                    </div>
                                    <div class="col-12 col-sm-12 col-md-12 col-lg-2 col-xl-2">
                                      <ValidationProvider name="Hastalık Değeri Türü" rules="required" v-slot="{ errors }">
                                        <div class="form-group">
                                          <label v-bind:for="'diseaseType'+index">Hastalık Değeri Türü</label>
                                          <input v-bind:id="'diseaseType'+index" type="text" class="form-control" name="diseaseType[]" v-model="input.type">
                                          <small class="font-weight-bold text-danger">{{ errors[0] }}</small>
                                        </div>
                                      </ValidationProvider>
                                    </div>
                          ...
                          <button class="btn btn-outline-primary rounded-0 btn-lg" type="submit">Hastalığı Güncelle</button>
                        </v-stepper-content>
                      </v-stepper-items>
                    </v-stepper>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      options: {
        url: process.env.apiBaseUrl + "panel/diseases/create-file/"+this.$route.params.id,
        headers: {
  ...
  async asyncData({params, error, $axios}) {
    try {
      ...
      const {data} = await $axios.get(process.env.apiBaseUrl + "panel/diseases/update/" + params.id)
      console.log(data);
  ...
  methods: {
    ...
    retrieveData(url) {
      ...
      this.$axios.get(`${process.env.apiBaseUrl}panel/datatables/${urlParam}?table=diseases_file&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,email,phone&where_column=diseases_id&where_value=${this.data._id.$oid}&joins=diseases_file`, {
        json: true,
    ...
    editDiseases() {
      ...
      this.$axios.post(process.env.apiBaseUrl + "panel/diseases/update/" + this.data._id.$oid, formData, {
        json: true,
        withCredentials: false,
        mode: 'no-cors',
        headers: {
