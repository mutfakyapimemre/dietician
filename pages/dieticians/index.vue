          ...
          <div class="search-box justify-content-center">
            <form onsubmit="return false" class="justify-content-center">
              <div class="form-group search-info w-100">
                <input type="text" class="form-control rounded-0" v-on:keyup.prevent="getDoctors()" v-model="search" placeholder="Diyetisyen Aramak İçin Buraya Yazın..."/>
              </div>
            </form>
          </div>
          <!-- /Search -->
    ...
    <section>
        <div class="row">
          <div class="col-12 col-sm-12 col-md-6 col-lg-9 col-xl-9">
            <div class="row">
              <!-- Doctor Widget -->
                <div v-if=" doctors !== null &&  doctors !== '' && doctors !== undefined && doctors.length>0" v-for="doctor in  doctors" class="col-12 col-sm-12 col-md-6 col-lg-3 col-xl-3">
                  <div class="h-100 profile-widget mx-1">
                    <div class="doc-img">
                    <nuxt-link tag="a" v-bind:to="'/profile/'+doctor.slug">
                      <img class="img-fluid" style="min-height:273px" v-bind:alt="doctor.name" v-bind:src="base_img_url+'/public/storage/'+doctor.profile_photo.img_url"/>
                    </nuxt-link>
                    </div>
...
export default {
  ...
  methods:{
    getDoctors(param){
      if(this.search !== null){
          this.$store.dispatch("getDoctors",{doctorsURL:"doctors?page="+this.pagination.current+"&search="+decodeURIComponent(this.search)}).then((response) => {
        ...
        if(param){
          ...
          this.$store.dispatch("getDoctors",{doctorsURL:"doctors?page="+this.pagination.current}).then((response) => {
            this.doctors = this.$store.getters.doctors.data
            this.pagination.current = this.$store.getters.doctors.current_page;
            this.pagination.total = this.$store.getters.doctors.last_page;
          })
