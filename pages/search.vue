          ...
          <div class="search-box justify-content-center">
            <form onsubmit="return false" class="justify-content-center">
              <div class="form-group search-info w-100">
                <input type="text" class="form-control rounded-0" v-on:keyup.prevent="getSearchs()" v-model="search" placeholder="Diyetisyen, Besin, Yemek Tarifi Ara"/>
              </div>
            </form>
          </div>
          <!-- /Search -->
...
export default {
  ...
  methods:{
    getSearchs(param){
      if(this.search !== null){
          this.$store.dispatch("getSearchs",{searchsURL:"searchs?page="+this.pagination.current+"&search="+decodeURIComponent(this.search)}).then((response) => {
        ...
        if(param){
          this.$store.dispatch("getSearchs",{searchsURL:param}).then((response) => {
            this.searchs = this.$store.getters.searchs.data
            this.pagination.current = this.$store.getters.searchs.current_page;
            this.pagination.total = this.$store.getters.searchs.last_page;
          })
        }else{
          this.$store.dispatch("getSearchs",{searchsURL:"searchs?page="+this.pagination.current}).then((response) => {
            this.searchs = this.$store.getters.searchs.data
            this.pagination.current = this.$store.getters.searchs.current_page;
            this.pagination.total = this.$store.getters.searchs.last_page;
          })
